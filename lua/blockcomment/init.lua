...
M.process_lines = function(from, to)
    ...
    local current_lines = vim.api.nvim_buf_get_lines(current_buf, from - 1, to, false)

    local code_is_commented = M.is_already_commented(current_lines, cleaned_comment_string)

    if code_is_commented == true then
        M.uncomment_lines(from, to, current_lines, comment_string)
    else
        M.comment_lines(from, to, current_lines)
    end
...
M.is_already_commented = function(lines, comment_prefix)
    for _, line in ipairs(lines) do
        local is_commented =
            line:match("^%s*" .. vim.pesc(comment_prefix)) ~= nil
            or line == ""
...
M.is_line_commented = function(line)
    ...
    local cleaned_comment_string = comment_string:gsub("%%s", "")
    return line:match("^%s*" .. vim.pesc(cleaned_comment_string)) ~= nil or line == ""
end



M.uncomment_lines = function(from, to, lines, comment_string)
    local current_buf = M.get_buffer()
    if not current_buf then
...
M.comment_lines = function(from, to, lines)
    local current_buf = M.get_buffer()
    if not current_buf then
        print("Error: Couldnt get buffer")
        return false
